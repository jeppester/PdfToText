			...
			switch  ( $preg_error )
			   {
				...
				case  PREG_INTERNAL_ERROR :
					error ( new PdfToTextException ( "PDF object extraction : the preg_match_all() function encountered an internal error." ) ) ;

				case  PREG_BACKTRACK_LIMIT_ERROR :
					error ( new PdfToTextException ( "PDF object extraction : backtrack limit reached (you may have to modify the pcre.backtrack_limit " .
							"setting of your PHP.ini file, which is currently set to " . ini_get ( 'pcre.backtrack_limit' ) . ")." ) ) ;

				case  PREG_JIT_STACKLIMIT_ERROR :
					error ( new PdfToTextException ( "PDF object extraction : JIT stack limit reached (you may disable this feature by setting the pcre.jit " .
							"setting of your PHP.ini file to 0)." ) ) ;

				case  PREG_RECURSION_LIMIT_ERROR :
					error ( new PdfToTextException ( "PDF object extraction : recursion limit reached (you may have to modify the pcre.recursion_limit " .
							"setting of your PHP.ini file, which is currently set to " . ini_get ( 'pcre.recursion_limit' ) . ")." ) ) ;

				case  PREG_BAD_UTF8_ERROR :
					error ( new PdfToTextException ( "PDF object extraction : bad UTF8 character encountered." ) ) ;

				case PREG_BAD_UTF8_OFFSET_ERROR :
					error ( new PdfToTextException ( "PDF object extraction : the specified offset does not start at the beginning of a valid UTF8 codepoint." ) ) ;

				default :
					error ( new PdfToTextException ( "PDF object extraction : unkown PREG error #$preg_error" ) ) ;
				...
				foreach  ( $ranges  as  $range )
				   {
					...
					// We put them in the DirectMap array, since there is only a one to one correspondance
				   	if  ( preg_match ( '/ < \s* (?P<from> [0-9a-f]+) \s* > \s* < \s* (?P<to> [0-9a-f]+) \s* > \s* \[(?P<subst> [^\]]+ )\] /imx',
									$range, $range_match ) )
					   {
					...
					else if  ( preg_match ( '/ <\s* (?P<from> [0-9a-f]+) \s*> \s* <\s* (?P<to> [0-9a-f]+) \s*> \s* <\s* (?P<subst> [0-9a-f]+) \s*> /imx',
									$range, $range_match ) )
					   {
						$from 		=  hexdec ( $range_match [ 'from'  ] ) ;
						$to 		=  hexdec ( $range_match [ 'to'    ] ) ;
		...
		if  ( isset ( self::$CachedMaps [ $map_name ] ) )
		...
		else
		   {
			...
			if  ( ! file_exists ( $file ) )
			   {
				if  ( PdfToText::$DEBUG )
					warning ( new PdfToTextException ( "Could not find CID table \"$map_name\" in directory \"" . PdfToText::$CIDTablesDirectory . "\"." ) ) ;
			    }
			else
				require ( $file ) ;
		...
		if  ( ! $this -> ImageResource )
		   {
			if  ( $this -> NoResourceCreated  &&  $image_type  ==  IMG_JPEG )
				...
				warning ( new PdfToTextException ( "No image resource allocated." ) ) ;
		...
		if  ( isset ( self::$Decoders [ $color_scheme ] ) )
		   {
			if  ( isset ( self::$Decoders [ $color_scheme ] [ $bits_per_component ] ) )
				...
				error ( new PdfToTextException ( "No decoding function has been implemented for image objects having the " .
						self::$DecoderNames [ $color_scheme ] . " color scheme with $bits_per_component bits per color component." ) ) ;
		    }
		else
			error ( new PdfToTextException ( "Unknown color scheme $color_scheme." ) ) ;
		...
		switch ( strtolower ( $color_space_name ) )
		   {
			...
			default :
				warning ( new PdfToTextException ( "Unsupported color space \"$color_space_name\"." ) ) ;
				return ( false ) ;
		    }
		...
		switch ( $bits_per_component )
		   {
			...
			default :
				warning ( new PdfToTextException ( "Unsupported bits per component : $bits_per_component." ) ) ;
				return ( false ) ;
		    }
...
class  PdfFaxImage		extends  PdfImage
   {
	...
	protected function  CreateImageResource ( $image_data )
	   {
		warning ( new PdfToTextException ( "Decoding of CCITT Fax image format is not yet implemented." ) ) ;
		//return ( imagecreatefromstring ( $image_data ) ) ;
	    }
    }
